...
epsilon = 16;
T = 300;
#n = 10**19; #doping concentration, in cm^-3
E = 0.2; #eV; 
k0 = np.sqrt(E/coef)
...
for n in [5*10**18, 10**19, 10**30]: #eV*nm, here 10^30 completely screen the Coulomb potential, equivalent to no trap;
#k1=0
    ...
    def V(x,y,z):
        r1 = np.sqrt((x+d)**2+(y-a/2)**2+z**2);
        return k1*np.exp(-r1/lD)/r1;
    ...
    I_list.append(100+10*(n/3/10**18-1));
    RTN.append((1-channel[-1]));
    nl.append(n)
    
with open('wave_function.txt','w') as file:
    
    file.write('z (nm)\t n = 5*10^18\t n = 10^19\t No trap\n')
